function [ct_in] = set_voi_mat(ct_in,A,y,x,sliceno,hei,wid,dep)
% [ct_in] = set_voi_mat(ct_in,A,y,x,sliceno,hei,wid,dep)
%--------------------------------------------------------------------------
...
if (isa(ct_in,'CT'))
    if (iscell(ct_in.slice))
        ...
        for ix = z1:z2
            ...
            tmp_slice(y1:y2,x1:x2)= A(:,:,k); 
            ct_in.slice{ix}= tmp_slice; 
%            ct_in.slice{ix}(y1:y2,x1:x2)= A(:,:,k);  is slower
            k = k+1;
        end
    else
        ct_in.slice(y1:y2,x1:x2,z1:z2) = A;
    end
else

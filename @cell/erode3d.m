function C= erode3d(A,B, resolution)
% function C= erode3d(A,B, resolution)
% function implements 3d erosion  function for cell array
% First input should be cell array of type int16,
% second should be erode3d window size 
...
if (cell_a)
    t= A{1}(1,1);
    if(isscalar(B))
        C = morph_mex_int16(A,resolution(1),resolution(2),resolution(3), B,1);
    else
